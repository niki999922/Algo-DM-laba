...
int n, m, qur, flag_left_son;
long long val, place, inf = LLONG_MAX-1;

void exet(int v, int tl, int tr, int pos)
{
    ...
    int tmp = (tl + tr) >> 1;
    if (pos <= tmp)
        exet((v << 1) + 1, tl, tmp, pos);
    else
        exet((v << 1) + 2, tmp + 1, tr, pos);
...
int main() {
	...
	while(fin >> str >> tmp) {
		if (str == "enter") {
			...
			val = inf;
			exet(0, 1, line_l + 1, place);
			fou << place << '\n';
		}
		if (str == "exit") {
			val=tmp;
			exet(0, 1, line_l + 1, tmp);
		}
	}
	fin.close();
	fou.close();
